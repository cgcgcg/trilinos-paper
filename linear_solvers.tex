...
framework \revised{to develop solvers} for linear systems and least-squares problems. This
framework \revised{is powerful because it} provides several \revised{algorithmic} abstractions
that facilitate code reuse and extensibility. \revised{These abstractions are at every level
...
\revised{smoothing for Maxwell and Darcy type problems} in the style of
...
performance gains for the triangular solve or inexact solves with ILU on GPUs\revised{; speedups of up to $4.8\times$ are observed when comparing ILU solve times on CPU and GPU.}
...
\revised{MueLu is easily extensible which makes it a good framework for the research and development of new multigrid preconditioning methods, e.g. different types of strength-of-connection or drop schemes.}
...
\revised{Many multi-physics problems are conveniently described by a block-structured matrix
...
The Teko library~\cite{Cyr2016a} provides interfaces for operators and preconditioners that are constructed from \revised{such} large physics-based sub-blocks.
...
\revised{Stratimikos does not support eigenvalue problems.
