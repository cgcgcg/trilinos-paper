...
it natively implements the direct solver KLU2 \cite{Davis2010a} as well as offers an uniform interface to third-party direct linear solvers such as CHOLMOD, MUMPS, Pardiso\_MKL, SuperLU, SuperLU\_MT, SuperLU\_Dist, and STRUMPACK, and it provides an interface to the on-node sparse direct solvers implemented in ShyLU.}
...
\revised{Many multi-physics problems are conveniently described as a block-structured matrix which allows easy access to the underlying matrices describing single physics or coupling between different types of physics.}
