...
\revised{All algorithms are implemented under base-class abstractions for the solver, direction, and line search objects.
...
any application.
\revised{Its programming interface supports modern computational
...
\revised{In contrast to other vector implementations, ROL's abstract linear algebra
...
\revised{stochastic problems}~\cite{shapiro2021lectures}, ROL includes sample-based
...
the $\ell^1$-norm are \revised{common in data science and machine learning~\cite{fuhg2024extreme}}.
...
ROL offers an application development kit \revised{for finite element models, called PDE-OPT}.
...
the values of input, output, and state variables for every sample of an ensemble and can take advantage of SIMD (single instruction, multiple data) operations. This type can then be used within the Tpetra solver stack as \revised{an argument for its scalar type template parameter, allowing instantiation of Tpetra data structures on the ensemble type to propagate ensembles through supported linear solvers and preconditioners}.
This approach \revised{results in reduced} computation time in four ways: the sample-independent data and computation can be reused for every sample of an ensemble, memory access patterns can be improved,
...
the convergence of the individual samples in order to decide when to terminate based on \revised{whether every sample within the ensemble has satisfied the chosen stopping criteria}.
...
Piro~\cite{osti_1231283} provides a simple and unified interface to nonlinear analysis tools in Trilinos, exposing capabilities for computing sensitivities, performing parameter continuation and bifurcation analysis, and solving constrained optimization problems. \revised{Piro wraps many of the common functionalities of Trilinos packages such as Belos, NOX, LOCA, Tempus and ROL, into a simple interface.}
%The main purpose of the package is to provide driver classes for the common uses of Trilinos nonlinear analysis tools.
In particular, Piro implements main driver classes for:
\begin{itemize}
	\item \emph{Linear/nonlinear solvers and sensitivity analysis} for transient and nontransient problems. As an example, this capability can be used to compute the discrete solution of a partial differential equation depending on a parameter and the sensitivity of a quantity of interest with respect to this parameter. Sensitivities can be computed in a forward or adjoint fashion, the latter being preferable in the presence of high-dimensional parameters. \revised{To this end, Piro performs forward and adjoint solves, relying on Belos, NOX and Tempus for the solution of linear, nonlinear and} transient problems.
...
\item \emph{Parameter continuation and bifurcation analysis.} \revised{The core capabilities for continuation and bifurcation analysis are} provided through the package LOCA. \revised{The Piro driver integrates these functionalities with linear, nonlinear, and time integration solvers, enhancing flexibility and usability. }
\end{itemize}
