...
This section describes Trilinos packages that provide tools for spatial and temporal discretization of integro-differential equations. Most discretization efforts in Trilinos have been devoted to implementing tools for mesh-based discretizations of PDEs with a focus on high-order finite elements. A notable exception is the research package Compadre (Section~\ref{sec:compadre}), which provides tools for meshless approximation of linear operators that can be used for the discretization of differential equations and for data transfer.
...
Panzer can provide utilities for application codes to build on, or can be used as a high-level application framework. Important capabilities include the following.
\begin{itemize}
