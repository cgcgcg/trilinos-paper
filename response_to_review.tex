...
\todo{
  AH: We may want to consider adding this reference. Although I generally dislike when reviewers ask for citations to their own work, if the reference is genuinely relevant, we should probably include it. I think it would be Ok to add this reference.
}
...
\Response{
  We forgo the inclusion of a detailed example as we believe that the included description matches the level of detail of the paper. Detailed examples of maps can be found in the Tpetra documentation. Yet, we have made sure to complement the description of \texttt{Map} objects.
}

\todo{Do we have any schematic figure about a Map? Even though this would make the paper longer, I agree with the reviewer that this is a key concept. So if we have any figure, it may help the readers if we include it.}
...
\todo{Should we mention Tpetra as a concorete implementation of this concept in the manuscript (similar to Stratimikos in the next bullet point)? \textbf{AH:} I agree with the reviewer that this sentence is not very clear. I thought about this, but I do not have a good concrete suggestion. Does one of you have a suggestion? I would suggest to slightly change this is in the paper.}
...
\todo{Perhaps, I missed this. But where is the term ``factory'' defined/explained?}
...
On the other hand, Anasazi itself implements Thyra and Tpetra interfaces.}

\todo{Could we incorporate this into the paper? Could we put a reference to Anasazi in that section?}
...
\Response{
  ...
  Instead, an always up-to-date overview of build/test configurations can be found in Trilinos' testing pipelines as part of the Trilinos GitHub repository.
\todo{Point to the respective location in the Trilinos repository.}}
