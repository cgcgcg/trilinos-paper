...
\Response{
  ...
  We are also actively working on improving the availability and correctness of our documentation and tutorial materials.
}
...
\Response{
  Trilinos can take a long time to compile due to the large number of capabilities.
  There are, however, multiple ways of achieving faster builds. 
  Clearly, compile time depends on the number of packages and Kokkos backends and whether tests and examples are built.
  Ensuring that only required features are enabled can significantly reduce build times.
  The underlying software stack is also a major factor.
  Compile times on certain GPU platforms can be quite long due this fact.
...
\Response{
  ...
  It will be archived before the end of the year and is therefore not mentioned in the paper.
  There are currently no pip or anaconda packages for its successor PyTrilinos2, but we will consider providing them.
}
