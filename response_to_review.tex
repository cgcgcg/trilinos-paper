...
\Response{We have deliberately chosen the term ``tools'', as Teuchos is precisely that:
a collection of helper and utility components designed to simplify common programming tasks and to ensure a consistent coding style across Trilinos packages.
Teuchos is not a programming interface in the conventional sense of defining an API boundary or abstraction layer for user interaction.
Each Trilinos package defines its own interface, while Teuchos provides reusable building blocks for recurring tasks
(such as memory management or access to BLAS/LAPACK routines)
implemented in a consistent and unified manner throughout the code base.
We have revised the description of Teuchos in the manuscript to clarify this distinction for the reader.}
...
Yet, we have made sure to complement the description of \texttt{Map} objects.}
...
\todo{Do we need to clarify this in the manuscript?}
...
\todo{Should we mention Tpetra as a concorete implementation of this concept in the manuscript (similar to Stratimikos in the next bullet point)?}
...
\Response{It may have been unclear that a solver manager provides a strategy for a single iterative method, like CG, GMRES, etc.  That has been clarified.}
...
\Response{We have added the following statement to indicate best case performance gains from using GPUs:}
...
\Response{We have rearranged the content of this section, such that we first describe Amesos2 with its solvers with general applicability and
...
\Response{This is Block Triangular Form. KLU \cite{Davis2010a} also relies on BTF which was
originally inspired Gilbert and Peierls algorithm (J. R. Gilbert and T. Peierls. Sparse partial pivoting in time proportional to arithmetic operations. SIAM
Journal on Scientific and Statistical Computing, 9(5):862–874, 1988). The phrase ``KLU'' is rather a pun than an abbreviation. It
comes from "Clark Kent" LU, as Gilbert Peierls algorithm is the LU algorithm that came before LU factorization became "Super"LU! We leave
this explanation out of the paper.}
...
\Response{Yes, Tacho and Basker are designed to be only node level solvers.}
...
\Response{Amesos2 implements one direct solver, KLU2,  and provides interfaces to direct solvers in third-party libraries as well as in the
Trilinos package ShyLU.
...
\Response{We have added more context to further describe the purpose of the Teko package.}
...
\Response{We have refrained from detailing build/text matrix configurations, since they are subject to continuous change,
in particular adding new hardware architectures when they become available.
An always up-to-date overview of build/text configurations can be found in Trilinos' testing pipelines as part of the Trilinos GitHub repository.
