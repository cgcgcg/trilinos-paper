...
\AtBeginDocument{%
  ...
  \renewcommand{\maketitle}{%
    \oldmaketitle
    \pagestyle{empty}% or plain
    \fancyhead{}%
    \fancyfoot{}%
  }%
}

% numeric citations to match the main doc
\citestyle{acmnumeric}

\usepackage{enumitem}
\setitemize{itemsep=0.0ex,topsep=-0.2ex,labelsep=0.5em}

% include some standard latex packages
\usepackage{geometry}
\geometry{a4paper, top=25mm, left=25mm, right=25mm, bottom=30mm, head=20mm}
...
\Response{
  ...
  Its parallel data structures --- distributed matrices and vectors --- are quintessential linear algebra objects, even if Tpetra also offers some functionality that extends beyond strict linear algebra.
  Overall, it still primarily covers distributed linear algebra, whereas, for instance, wrappers to distributed-memory communicators live in Teuchos.
  This does not mean that it can only be used in the context of the solution of linear systems.
  For example, finite element discretizations provided by Intrepid2 and Panzer rely on Tpetra data structures or the work mentioned by the reviewer.
  Optimization packages such as ROL can leverage Tpetra objects.
  In our view, ``linear algebra'' is a field on its own and does not equate to ``solution of linear systems of equations''.
  Therefore, Tpetra is part of Trilinos' ``Core'' product area, while all linear solvers are collected in their own product area.
  We have added an additional statement to further stress how crucial Tpetra is for Trilinos:
}
\Changes{Due to the fundamental nature of MPI distributed data structures, the majority of Trilinos packages depend directly or indirectly on Tpetra.}
...
\Response{
  We thank the reviewer for this comment, as well as the ones below; they align closely with feedback from the broader Trilinos community.
  The Trilinos team is in the process of removing the old Epetra package stack, and the presentation of Trilinos capabilities and their documentation should become much clearer once everything is built around a single sparse linear algebra package.
  We are also actively working on improving the availability and accuracy of our documentation and tutorial materials.
}
...
\Response{
  Similar to the previous comment, this feedback echoes what we frequently hear from the Trilinos community. Trilinos can take a long time to compile because of the breadth of capabilities it offers, though several approaches can accelerate builds. Compile time depends on the selected packages, Kokkos backends, and whether tests or examples are enabled, so tailoring a configuration to the needed features often shortens builds considerably. The underlying software stack is another major factor, and GPU platforms in particular tend to exhibit long compile times. Trilinos runs CPU and CUDA builds for every PR in CI, with turnaround typically about an hour thanks to \texttt{ccache}. We will consider documenting best practices for fast builds on the Trilinos web page or GitHub wiki.
}
...
\Response{
  We agree with the reviewer that robust Python support and straightforward installation via \texttt{pip} or Anaconda would enhance Trilinos' usability. The current Anaconda package is based on PyTrilinos and the legacy Epetra stack; it will be archived before the end of the year and is therefore not mentioned in the paper. There are presently no \texttt{pip} or Anaconda packages for the PyTrilinos2 stack, but we are investigating how to provide up-to-date distributions aligned with current Trilinos releases.
}
...
\Response{
  We agree this is a valid point: parts of the paper can leave the impression of being terse and might require background knowledge on the underlying mathematical methods, even though we aimed to keep the text accessible to a broad audience. Given the variety of algorithms included in Trilinos, it might be too big of a task to describe all algorithms in full detail in a single manuscript, especially given page limits of scientific journals. As the reviewer observes, the manuscript already exceeds 30 pages. The main goal of this manuscript is to provide an update on recent changes to the overall Trilinos project, in particular its adoption of the Kokkos programming model to enable performance portability across hardware architectures. Some packages tackle well-known concepts, whereas others implement more specialized algorithms that are less broadly familiar, making them harder to describe comprehensively within the space available. To maintain a finite scope, the reader is referred to the packages' documentation or individual publications for details on concrete algorithms in specific software packages, and we have aimed to cite sufficient references so readers can explore each topic more deeply. Within the constraints of this paper, we will incorporate the reviewer's specific comments wherever feasible.
}


\Comment{Overall, I believe the manuscript is definitely worth considering for publication as long as the detailed comments listed below are taken into consideration.}
...
\Response{We have added short explanations for the purpose of execution and memory spaces. We now write:}

\Changes{
  In particular, the use of \texttt{execution spaces} (where does code run) and \texttt{memory spaces} (where does data reside) as parameters of data structures and algorithms is a common thread throughout Trilinos.
}
...
\Response{
  We used ``high-level'' to describe algorithms that build on top of or use basic algorithms and kernels, which we referred to as ``low-level''. However, we never precisely defined this distinction, and we agree that the terminology lacks rigor and generality. Because the distinction is unnecessary here, we removed both ``high-level'' and ``low-level''.
}

\Comment{Page 5, par 1 and 2 – automatic differentiation is mentioned in both the ``Non-linear Solvers and Analysis Tools'' and ``Discretization Tools''. I would suggest that already at this stage an explanation of the difference between these AD is provided}

\Response{
  AD plays an important role in many aspects of computational science, so several Trilinos packages rely on it. Section 4.3 introduces Sacado, Trilinos' AD framework within ``Nonlinear solvers and analysis tools''. The discretization packages make use of Sacado as well, which is why AD is mentioned in that paragraph. We have rephrased the text to clarify that the AD used by the discretization schemes is the same framework described in ``Nonlinear solvers and analysis tools'', rather than a separate variant.
}
...
\Response{
  Currently, there no publication focusing specifically on the batched BLAS routines in KokkosKernels, so we now cite reference~\cite{10.1145/3431921} as a comprehensive overview of batched BLAS techniques.
  We also replaced the phrase ``general capabilities'' with an explicit list of the algorithm classes that MueLu relies on.
}
...
\Response{
  Thank you for pointing this out. We should have stated more clearly that Kokkos and its subprojects are now developed independently of Trilinos. For convenience, Kokkos releases are periodically snapshotted into Trilinos. We have added a sentence to make this explicit:
}

\Changes{While the development of Kokkos Core has originated as a package of Trilinos, Kokkos has become an independent project in 2015
...
\Response{
  We have deliberately chosen the term ``tools'', as Teuchos is precisely that: a collection of helper and utility components designed to simplify common programming tasks and to ensure a consistent coding style across Trilinos packages. Teuchos is not a programming interface in the conventional sense of defining an API boundary or abstraction layer for user interaction. Each Trilinos package defines its own interface, while Teuchos provides reusable building blocks for recurring tasks (such as memory management or access to BLAS/LAPACK routines) implemented in a consistent and unified manner throughout the code base. We have revised the whole description of Teuchos in the manuscript to clarify this distinction for the reader.
}
...
\Response{
  Tpetra focuses on distributed sparse linear algebra and dense vectors, but not on distributed dense matrices.
  Dense matrix functionality, including batched dense linear algebra, is provided by Kokkos Kernels at the node (intra-rank) level.
  Tpetra itself does not implement distributed dense matrices because such use cases are uncommon in large-scale PDE-based simulations that motivated Tpetra’s design.
  We note though, that a \texttt{MultiVector} is a collection of multiple vectors. It can therefore be used to represent a (usually tall and skinny) dense matrix.
  Given Tpetra's focus on sparse linear algebra, it is not recommended to represent large distributed dense matrices by Tpetra objects.}

\Comment{Page 7, par 1 -- Could you give a simple example of ``Map data structure''.}

\Response{
  We have complemented the description of \texttt{Map} objects and have added an illustrative example of a vector distributed among two MPI processes in Figure~\ref{fig:Map}.
  A detailed discussion and further examples of maps can be found in the Tpetra documentation.
}

\Comment{Page 7, 1st bullet point -- Why associating BLAS-1 like kernels with TSQR?}

\Response{
  Thank you for this comment. The bullet describes the \texttt{Vector} and \texttt{MultiVector} classes provided by Tpetra. \texttt{MultiVectors} are used in Trilinos to hold tall-and-skinny dense matrices. This explains why TSQR is listed under this bullet.
}
...
\Response{
  The listed kernels need to be aware of the specific data storage format as they directly access data buffers. We tried to clarify by making this explicit in the text.
}
...
\Response{Yes, this provides an abstract layer. We have slightly reformulated these sentences to clarify this point:}

\Changes{%
  The abstractions hide concrete linear algebra and solver implementations so that algorithm development is separated from the implementation of the underlying kernels, allowing the same code to be written once and reused across many application codes. For example, one could write a Krylov solver and, because the low-level kernels remain behind the abstraction layer, use it with multiple concrete linear algebra implementations such as Tpetra in Trilinos or the vector/matrix objects in PETSc~\cite{petsc-web-page}.
}
...
\Response{Thyra provides abstract interfaces. Tpetra is one of the possible concrete implementations of these interfaces. We have modified the paragraph as follows:}

\Changes{The abstractions hide concrete linear algebra and solver implementations so that algorithm development is separated from the implementation of the underlying kernels, allowing the same code to be written once and reused across many application codes. For example, one could write a Krylov solver and, without modifications to computational kernels that are hidden behind the abstraction layer, use it with multiple concrete linear algebra implementations such as Tpetra in Trilinos or the vector/matrix objects in PETSc.}
...
\Changes{[...] The solver interfaces allow one to wrap any concrete linear solver or preconditioner for use with the ANAs. As a concrete implementation of this concept, the Stratimikos package wraps all of the Trilinos preconditioners and linear solvers in Thyra abstractions. [...]}
...
While Tpetra offers distributed linear algebra, it does not compute the layout of the distribution itself. We slightly rephrased the sentence:}

\Changes{%
  Zoltan2 supports the use of both distributed-memory parallelism and on-node parallelism on CPUs and GPUs by building on Kokkos.
}
...
\Response{
  Thank you for this comment. We have slightly restructured the paragraph to better convey the fact that the following sentence gives examples of traits classes.
}
...
\Response{We have provided further detail on why abstractions provided by the Belos solver framework could be considered powerful.}
...
\Response{
  We recognized that it may have been unclear that a solver manager provides the strategy for a single iterative method, such as CG or GMRES, and have clarified the text.
}
...
\Response{
  For clarity within the context of this paper, we now refer to the types of physics associated with the H(curl) and H(div) discretizations instead of naming the function spaces directly.
}
...
\end{quote}
  highlights several important properties of FROSch. In the next paragraph, we first focus on the algorithmic aspects that enable parallel scalability and algebraicity. The sentence in question, now slightly reformulated, opens the next paragraph and highlights the broad applicability. We find it natural to separate these two aspects into distinct paragraphs and hope the referee agrees with this structure. We hope that this clarifies.
}
...
\Response{Thank you. We have added the following statement to indicate best case performance gains from using GPUs:}

\Changes{[...] speedups of up to $4.8\times$ are observed when comparing ILU solve times on CPU and GPU [...]}
...
\Response{Performance and scaling studies on the El Capitan system are work in progress. When ready, we plan to publish them in a separate, fully MueLu-centered manuscript.}
...
\Response{Thank you. We have rearranged the content of this section, such that we first describe Amesos2 with its solvers with general applicability and
then detail more application-targeted solvers in ShyLU.
We have added statements to make this distinction clear to the reader.  While the motivation for the development of solvers within ShyLU are
application specific, they are reasonably general purpose. Basker, for example, could be used in the same context as KLU. It is a
non-supernodal direct solver that relies on Block Triangular Form similar to KLU. It works even if this structure is not present. However, it is not
the most efficient. Similarly, Tacho has been optimized for special class of problems. We have clarified this. }
...
\Response{
  This is block triangular form (BTF). KLU~\cite{Davis2010a} also relies on BTF and was originally inspired by the Gilbert--Peierls algorithm (J.~R. Gilbert and T. Peierls, ``Sparse partial pivoting in time proportional to arithmetic operations,'' SIAM Journal on Scientific and Statistical Computing, 9(5):862--874, 1988). The phrase ``KLU'' is more of a pun than an abbreviation: it comes from "Clark Kent" LU, as the Gilbert--Peierls algorithm preceded the ``SuperLU'' factorization. We leave this explanation out of the paper.
}
...
\Response{You are right, Tacho and Basker are designed to be only node level solvers. We have clarified this as follows:}

\Changes{Basker~\cite{Basker2017} is a shared-memory sparse direct solver [...]}

\Changes{Tacho is a shared-memory sparse direct solver [...]}
...
\Response{Thank you for this comment. Amesos2 implements one direct solver, KLU2,  and provides interfaces to direct solvers in third-party libraries as well as in the
Trilinos package ShyLU. We have clarified that in the description of the Amesos2 package.}
...
\Response{
  We added context at the start of the section to better describe the purpose of the Teko package and hope this clarifies its scope for the reader.
}
...
\Response{The Stratimikos package was designed with the solution of linear systems \(Ax=b\) in mind,
allowing access to a wide variety of linear solvers and preconditioners in a uniform fashion.
While it is true that it could be extended to allow for eigenvalue problems \(Ax=\lambda x\),
Anasazi is the only eigensolver package in Trilinos.
On the other hand, Anasazi itself implements Thyra and Tpetra interfaces. We have added the following statements to the description of Stratimikos:}

\Changes{Stratimikos does not support eigenvalue problems. Anasazi has, however, a Thyra interface and can therefore be used with different linear algebra backends.}

\noindent\textbf{Section 4}

\Comment{Page 14, subsec 4.1, par 1 -- ``custom implementations'' of what? Please explain.}

\Response{NOX algorithms are built on abstract base classes for solvers, directions, line searches and stopping criteria. Users can write derived classes for each of these base classes and register them with NOX. This allows users to fully customize the algorithm. The statement has been expanded on to clarify.}
...
\Response{We agree that the original statement was a bit too strong, so we reformulated it as follows:}

\Changes{Its programming interface supports modern computational hardware, including heterogeneous multi-core systems.}
...
\Response{While ROL requires a limited subset of operations provided by Thyra vectors,
Thyra and other packages do not provide the concept of duality pairings for vector spaces,
which is needed for efficient infinite-dimensional optimization, such as optimization with
...
\Response{In this context, ``stochastic problems'' is a better term.  We have changed this accordingly.
By ``risk-averse problems,'' we meant stochastic optimization problems that
minimize risk measures such as conditional value at risk.}

\Comment{Page 15, subsec 4.2, 3rd bullet point -- ``data science and machine learning'' Perhaps a reference here? LASSO? Else?}

\Response{As an example, we added reference~\cite{fuhg2024extreme}, where the authors combine neural network-based data-driven constitutive modeling with sparse regression.
}
...
\Response{This capability of ROL is geared toward finite elements.
To avoid confusion, we have updated the text to clearly limit the
scope to finite element methods.}
...
\Response{
  We have revised the text to include more specific information, but we prefer to keep the description of the Piro package concise.
}
...
\Response{
  We agree that a discussion of performance results could make the manuscript more interesting for readers with an interest in particular algorithms of packages of Trilinos.
  ...
  Additionally, there is currently no consistent set of benchmark tests providing comparable performance results across the various solvers.
  A ``fair'' comparison between different solvers, spanning distinct implementations and even different classes of algorithms, would therefore be a difficult endeavor and remains beyond the scope of this paper.
  We agree that assembling such a comparison would make for an interesting dedicated publication.
  Given the current length of the manuscript, we prefer to omit this material here and hope that the referee will accept this decision.
...
\Response{
  We considered including a table of build/test matrix configurations, but ultimately refrained because they are subject to continuous change, especially when new hardware architectures become available.
  Instead, an always up-to-date overview of build/test configurations can be found in Trilinos' testing pipelines as part of the Trilinos GitHub repository at \url{https://github.com/trilinos/Trilinos/blob/master/.github/workflows/AT2.yml}. We have clarified this in the manuscript as follows:
}
\Changes{An always up-to-date overview of the configurations used for automated testing of pull requests against Trilinos can be found in the source code repository\footnote{\url{https://github.com/trilinos/Trilinos/blob/master/.github/workflows/AT2.yml}}.}

\Response{
  There is no pre-defined process for including external software into Trilinos. Rather, we invite potential contributors to contact the Trilinos product owners to discuss. To clarify this, we have added the following statement:
}
\Changes{Questions regarding the inclusion of new packages should be discussed with the Trilinos leadership team.}
...
\Response{
  Please see our previous response. Showing performance results for FROSch, MueLu and ROL might be of interest to some readers. Others might want to see results illustrating Ifpack2, Kokkos Kernels, Amesos2, Teko, Anasazi, NOX, Intrepid2, Panzer or Sacado. Selecting an appropriate subset of capabilities that truly represents the breadth of capabilities of Trilinos is very difficult. We regret to reiterate that we must refrain from adding specific results, as such a discussion remains outside the scope of this paper. We agree that it would be a valuable topic for a dedicated discussion elsewhere.
}

\Comment{The manuscript is a high-quality overview of a critical scientific infrastructure. With the addition of quantitative benchmarks and minor editorial improvements, it will serve as a lasting reference for both developers and end-users in the HPC community.}
