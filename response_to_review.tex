...
\Response{
  ...
  Ensuring that only required features can significantly reduce build times.
...
\Response{
  \todo{Figure out if there is a publication that covers batched BLAS specifically.}
...
\Response{We added a reference to ``Extreme sparsification of physics-augmented neural networks
for interpretable model discovery in mechanics,'' by Fuhg, Jones, and Bouklas (2024), where the
authors combine neural network-based data-driven constitutive modeling with sparse regression.
}
